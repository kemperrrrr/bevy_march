...
fn get_color(march: MarchSettings, res: MarchResult) -> vec3<f32> {
    ...
    // Calculate reflection if material is metallic
    if metallic > 0.01 {
        var reflected = march;
        ...
        if reflection_res.distance < 0.1 {
            ...
            albedo = mix(albedo, reflection_mat.base_color, metallic);
        } else {
            albedo = mix(albedo, skybox(reflected.direction), metallic);
        }
    }
    ...
    kD = kD * (1.0 - metallic);  // Changed from *= to explicit multiplication
